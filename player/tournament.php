...
if (!$ass_id && $session_user_id) {
    $get_ass = $con->query("SELECT ass_id FROM tbl_association_players WHERE user_id = '$session_user_id' LIMIT 1");
...
if (mysqli_num_rows($sql) > 0) {
    while ($row = mysqli_fetch_assoc($sql)) {
        ...
        // 🔹 Determine player's application status
        $status_row = mysqli_fetch_assoc($con->query("
            SELECT status 
            FROM tbl_tourna_application AS ta
            INNER JOIN tbl_profile AS p ON ta.prof_id = p.prof_id
            INNER JOIN tbl_association_players AS ap ON p.user_id = ap.user_id
            WHERE ta.tourna_id = '$tourna_id'
            AND ap.ass_id = '$ass_id'
            AND p.user_id = '$session_user_id'
            AND ta.ev_id = '$ev_id'
            LIMIT 1
        "));

        if ($status_row) {
            $status = ($status_row['status'] == '1') ? "Joined" : "Applied";
        } else {
            $status = "";
        }

        // ✅ Get tournament status directly from tbl_tournament (numeric)
...
.status-badge.joined { background-color: hsla(70, 100%, 65%, 1.00); }
.status-badge.applied { background-color: #444; }
