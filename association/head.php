...
if($session_user_type != 2){
    ...
    $user = $con->query("SELECT * FROM tbl_user AS u LEFT JOIN tbl_association_staff AS a ON u.user_id=a.user_id WHERE username ='$session_username'");
    $user = mysqli_fetch_assoc($user);

    $_SESSION['association_id'] = $user['ass_id'];

    if(!isset($_GET['e_id'])){
        $sql = $con->query("SELECT * FROM tbl_event WHERE ev_status='1'");
        if(mysqli_num_rows($sql) <= 0){
            include("../noeventmsg.php");
            die();
        }
    }else{
        $event_id = $_GET['e_id'];
