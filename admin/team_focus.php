...
$id = $_SESSION['tournament_code'] ?? null;
$team_id = $_GET['id'] ?? 0;

/* TOURNAMENT */
$sql = $con->query("SELECT s.name AS sport_name, gm.name, gm.scoring 
...
$tourna = mysqli_fetch_assoc($sql) ?? ['name'=>'Unknown','sport_name'=>'Unknown','scoring'=>1];

/* RECENT MATCHES FIXED QUERY */
...
");
$team = mysqli_fetch_assoc($team) ?? ['name'=>'Unknown Team','img_logo'=>'','place'=>null,'disqualify'=>0];

/* PLAYER COUNT */
$player = $con->query("SELECT COUNT(*) AS count FROM tbl_team_players WHERE team_id ='$team_id'");
$player = mysqli_fetch_assoc($player) ?? ['count'=>0];

/* WIN / LOSS FIXED SQL LOGIC */
$record = $con->query("
    SELECT 
        SUM(winner = $team_id) AS win, 
        SUM(winner != $team_id AND winner IS NOT NULL) AS lose 
    FROM tbl_matches 
    WHERE tourna_id='$id' AND (team1 = $team_id OR team2 = $team_id)
");
$record = mysqli_fetch_assoc($record) ?? ['win'=>0,'lose'=>0];
...
if ($team['place'] !== null) {
    ...
    if ($team['disqualify'] == 1) {
        ...
        switch ($team['place']) {
            case 0: $place = "Champion"; break;
            case 1: $place = "1st Place"; break;
            case 2: $place = "2nd Place"; break;
            case 3: $place = "3rd Place"; break;
            case 4: $place = "4th Place"; break; // fixed
            default: $place = $team['place']."th Place";
        }
    }
    ...
    <div class="body">
        ...
        <div class="recent_match">
            ...
            <?php while ($row = mysqli_fetch_assoc($recent_match)) {
                ...
                /* TEAM 1 INFO */
                $team1 = mysqli_fetch_assoc($con->query("
                    SELECT name, img_logo 
                    FROM tbl_team AS t 
                    LEFT JOIN tbl_association AS a ON t.ass_id = a.ass_id 
                    WHERE team_id = '".$row['team1']."'
                ")) ?? ['name'=>'Unknown','img_logo'=>''];

                /* TEAM 2 INFO */
                $team2 = mysqli_fetch_assoc($con->query("
                    SELECT name, img_logo 
                    FROM tbl_team AS t 
                    LEFT JOIN tbl_association AS a ON t.ass_id = a.ass_id 
                    WHERE team_id = '".$row['team2']."'
                ")) ?? ['name'=>'Unknown','img_logo'=>''];

                /* SCORE */
                $match_id = $row['match_id'];
                if ($tourna['scoring'] == 1) {
                    $score = mysqli_fetch_assoc($con->query("
                        SELECT team1, team2 
                    ...
                    $score = mysqli_fetch_assoc($con->query("
                        SELECT SUM(winner='team1') AS team1, SUM(winner='team2') AS team2 
                        FROM tbl_score_match 
                        WHERE match_id='$match_id'
                    ")) ?? ['team1'=>0,'team2'=>0];
                }
            ?>
                ...
                <li class="match_card">
                    <div class="match_meta">
                        <span>Start: 
                            <?php echo ($row['start_date'] ? date('M j, Y h:i A', strtotime($row['start_date'])) : "-"); ?>
                        </span>
                        <span>End: 
                            <?php echo ($row['end_date'] ? date('M j, Y h:i A', strtotime($row['end_date'])) : "-"); ?>
                        </span>
                    </div>
                    ...
                    <div class="match_content">
                        ...
                        <div class="scoreboard">
                            <h2><?php echo $score['team1']." - ".$score['team2']; ?></h2>
                            <p>VS</p>
                        </div>
