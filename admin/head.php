...
if($session_user_type != 1){
    ...
    if(!isset($_GET['e_id'])){
        ...
        if(mysqli_num_rows($sql) <= 0){
            $sql = $con->query("SELECT * FROM tbl_event ORDER BY ev_id DESC");
        }
    }else{
        $event_id = $_GET['e_id'];
        $sql = $con->query("SELECT * FROM tbl_event WHERE ev_id='$event_id'");
        $con->query("UPDATE tbl_event SET ev_status='0'");
        $con->query("UPDATE tbl_event SET ev_status='1' WHERE ev_id='$event_id'");
    }
